...
import public.db_insert as DB
import shixin_spider.depend.table as config
from public.share_func import *
# from depend.shixin_python_sql import *
...
class ShiXinSpider(object):
    ...
    def saveItems(self):
        ...
        if valid_num:
            DB.dbInsert('t_shixin_valid', config.COLUMN_VALID, self.valid_items)
        if invalid_num:
            DB.dbInsert('t_shixin_invalid', config.KEY_CONVERT_INVALID, self.invalid_items)
    ...
    def getStartID():
        """从json文件读取启动id"""
        with open('./depend/start_id.json', 'r') as f:
    ...
    def saveClawedID(sys_id):
        """将启动id保存到json文件"""
        with open('./depend/start_id.json', 'w') as f:
