...
def getIP():
    ...
    b=random.randint(128,233)
    c = random.randint(128, 223)
    d = random.randint(128, 233)
    #print '{0}.{1}.{2}.{3}'.format(a,b,c,d)
    print '%d.%d.%d.%d'%(a, b, c, d)

def getImagetamp(length=13):
    """
    获得指定长度的时间戳
    :param length:
    :return:
...
def removeAllFiles(dires):
    ...
    :param dires: 目录列表
    :return:
    os.listdir(dire)列出指定目录下的所有文件和子目录，包括隐藏文件，并以列表方式打印
    """
    #path='D:/workplace/pratice/spider_Book'
...
#makeDirs(dirs=['clawed2_log','clawed3_log'])
