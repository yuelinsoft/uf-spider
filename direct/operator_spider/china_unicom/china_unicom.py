...
import gevent
from gevent import monkey;monkey.patch_all(socket=True, dns=True, time=True,
                                           select=True,thread=False, os=True,
                                           ssl=True, httplib=False, aggressive=True)
from requests.utils import dict_from_cookiejar
from threadpool import ThreadPool, makeRequests

from .depend import table as Table
from .depend.param_date import getDateSeq
from public import (
    getUserAgent,
    Request,
    getTimestamp,
...
class ChinaUnicom(object):
    ...
    def loginSys(self):
        ...
        def sysCheckLogin():
            ...
            if response:
                ...
                return dict(code=4000, data=[], desc='sysCheckLogin',
                            func='sysCheckLogin')
        # end
        ...
        def loginByJS():
            ...
            if response:
                ...
                return dict(code=4000, data=[], desc='loginByJS',
                            func='loginByJS')
        # def
        ...
        def judgeLogin(response):
            ...
            try:
                ...
                if code in code_hash.keys():
                    self.cookies.update(dict_from_cookiejar(response.cookies))
                    return dict(code=code_hash[code], desc='judgeLogin',
                                data=[])
                else:
    ...
    def getCallInfo(self):
        ...
        def clawMonthCall(month):
            ...
            try:
                ...
                print('clawMonthCall 解析错误')
                # return returnResult(4100, [], desc='clawMonthCall 解析错误')
                return
            if 'errorMessage' in page_json.keys():  # 存在错误
                print('clawMonthCall 网络错误')
                # return returnResult(4000, [], desc='clawMonthCall 网络错误')
                return
            else:
        ...
        def coroutineClawPageCall(date_tuple, page_no=1, resend = 2):  #完成单次请求[存在网络繁忙则重传]
            ...
            if response:
                ...
                pass
                # returnResult(4000, [], desc=unicode('获取电话记录网络错误'))
        # def
            ...
            try:
                ...
                for record in results:
                    ...
                    for k, v in record.items():
                        if k in Table.KEY_CONVERT_CALL.keys():
                            column_name = Table.KEY_CONVERT_CALL[k]
                            temp[column_name] = v
    ...
    def saveItems(self):
        ...
        if valid_num:
            dbInsert(Table.TABEL_NAME_1, Table.COLUMN_USER, self.user_items)
        if invalid_num:
            dbInsert(Table.TABLE_NAME_2, Table.COLUMN_CALL, self.call_items)
...
def chinaUnicomAPI(phone_attr):
    ...
    if login['code'] != 2000:
        return returnResult(login['code'], data={}, desc='spider.loginSys()') # 返回登陆错误信息
    else:
        spider.getUserInfo()
        spider.getCallInfo()
        # spider.saveItems()
