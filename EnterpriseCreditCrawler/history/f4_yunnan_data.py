...
class CompanyInfo(CreditInfo):
    ...
    def basicinfo_execute(self, pageSoup):
        ...
        if info != []:
            info[0] = common.basicinfo_dict(info[0], u'云南省')
...
def main(**kwargs):
    ...
    headers_info = {
        ...
        # 'Referer': 'http://www.ynaic.gov.cn/notice/search/ent_info_list',
    ...
    html = url_requests.get(url, headers=headers_info, verify=False)
    ...
    execute_d = ['c_mortgage_execute', 's_creditor_execute',
                 'mortgage_execute']
    if cls.qyxx_mortgage_basic != []:
        for each in cls.qyxx_mortgage_basic:
    ...
    # 汇总数据到字典
    results = cls.returnData()

    return results

if __name__ == '__main__':
    # b_list = 'http://gsxt.ynaic.gov.cn/notice/notice/view?uuid=dosed.1oinwZgvWlQJza8K4JOHfgVnSw&tab=01'# 深圳前海融金所投资管理有限公司昆明分公司
    # b_list = 'http://gsxt.ynaic.gov.cn/notice/notice/view?uuid=FJnHSSp__qDZuWY65w38fkehyrNSgYrP&tab=01'# 昆明市五华区腾讯电脑配件经营部(经营异常信息)
    # b_list = 'http://gsxt.ynaic.gov.cn/notice/notice/view?uuid=E0Y4YBYrYYl2JzX7AYfcu6cgXjrAfm3M&tab=01'# 丽江腾讯旅行社有限公司(股东信息，变更信息，主要人员信息)
    # b_list = 'http://gsxt.ynaic.gov.cn/notice/notice/view?uuid=kZsTLGWSyOskjrk_sOHQP5tKlH.B_Zh9&tab=01'# 云南盐化股份有限公司(分支机构【翻页】)
    b_list = 'http://gsxt.ynaic.gov.cn/notice/notice/view?uuid=FJnHSSp__qDY1nO7oOm681df_s.afxUV&tab=01'# 昆明广旭宇畜牧有限公司(股权出质登记信息，司法协助公示信息)
    # b_list = 'http://gsxt.ynaic.gov.cn/notice/notice/view?uuid=FJnHSSp__qCausIgcnl.hIptKsyd8Ncu&tab=01'# 禄劝鑫磊石材厂(抵押物)
    # b_list = 'http://gsxt.ynaic.gov.cn/notice/notice/view?uuid=29Xff1YozUdFtZTZSeooB5.JZbl11Ewc&tab=01'# 宣威市阳光农资有限责任公司(行政处罚信息)
    main(id_tag=b_list)
